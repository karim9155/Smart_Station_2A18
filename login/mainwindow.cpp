    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->comboBox_2->addItems(B1.lister());
    ui->label_pic->hide();
}
...
{
    ui->label_pic->show();
    QPixmap pix("C:/Users/azert/Desktop/2eme/Projet C++/Login/login/map.png");
...
{
    QString matricule =ui->comboBox_2->currentText();



          QSqlQuery c;
          c.prepare("select * from BUS where matricule ='"+matricule+"'");
         if(c.exec())
         {
             while (c.next()) {
                  ui->le_type_2->setText(c.value(1).toString());
                  ui->le_marque_2->setText(c.value(0).toString());

                  ui->le_puissance_2->setText(c.value(3).toString());
                  ui->le_matricule_2->setText(c.value(2).toString());
...
{
    ui->tab_Bus->setModel(B1.triermatricule());
}
